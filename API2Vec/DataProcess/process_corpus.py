...
import pandas as pd
from DataProcess.process_corpus_without_count import roam_without_count, roam_without_count_analysis
from DataProcess.process_corpus_without_time import roam_without_time, roam_without_time_analysis
from DataProcess.process_corpus_without_time_count import roam_without_count_time, roam_without_count_time_analysis
...
def analysis_infos(infos, title="Global"):
    ...
    for info in infos:
        ...
        (g_edge_cover, s_tag_walk_count, s_tag_node_cover, s_tag_edge_cover, s_tag_path_len) = info
        ...
        tpg_edge_cover.extend(g_edge_cover)

    # tag_infos['tag_node'] = tag_infos['tag_node'][:max_count]
    # tag_infos['tag_edge'] = tag_infos['tag_edge'][:max_count]
    # tag_infos['tag_edge_unique'] = tag_infos['tag_edge_unique'][:max_count]

    tag_walk_count_infos['tag_walk_count'] = tag_walk_count_infos['tag_walk_count'][:max_count]

    tag_rw_infos['tag_node_cover'] = tag_rw_infos['tag_node_cover'][:max_count]
    tag_rw_infos['tag_edge_cover'] = tag_rw_infos['tag_edge_cover'][:max_count]
    tag_rw_infos['tag_path_len'] = tag_rw_infos['tag_path_len'][:max_count]

    tpg_edge_cover = tpg_edge_cover[:max_count]
    ...
    pd_tpg_edge_cover = pd.DataFrame(tpg_edge_cover)

    pd_tag_infos.to_excel(f'./Analysis/ablation/{title}_tag_infos.xlsx', index=False)
    pd_tag_walk_count_infos.to_excel(f'./Analysis/ablation/{title}_tag_walk_count_infos.xlsx', index=False)
    pd_tag_rw_infos.to_excel(f'./Analysis/ablation/{title}_tag_rw_infos.xlsx', index=False)
    pd_tpg_edge_cover.to_excel(f'./Analysis/ablation/{title}_tpg_edge_cover.xlsx', index=False)
    with open('./Analysis/ablation/result.txt', 'a', encoding='utf-8') as f:
        f.write(f'\n\n{title}\n\n')
        # f.write(f'{pd_tag_infos.describe()}\n\n')
        f.write(f'{pd_tag_walk_count_infos.describe()}\n\n')
        f.write(f'{pd_tag_rw_infos.describe()}\n\n')
        f.write(f'{pd_tpg_edge_cover.describe()}\n\n')
...
def load_corpus_pid(config, pid_api_seqs, tpg_datas, tag_datas, labels):
    ...
    if(config.model_type == 'ml'):
        if 'count' in config.params:
            if 'notime' in config.params: # API2Vec without time index
                with multiprocessing.Pool(processes=config.workers) as p:
                    corpus = list(tqdm(p.imap(roam_without_time, inputs), total=len(inputs), desc='roam_without_time'))
            else: # API2Vec
                with multiprocessing.Pool(processes=config.workers) as p:
                    corpus = list(tqdm(p.imap(random_walk_main, inputs), total=len(inputs), desc='API2Vec 游走进度'))
                    # corpus = list(tqdm(p.imap(random_walk_main_infos, inputs), total=len(inputs), desc='random_walk_main_infos'))
        elif 'count' not in config.params: 
            if 'notime' in config.params: # API2Vec without freq + time index
                with multiprocessing.Pool(processes=config.workers) as p:
                    corpus = list(tqdm(p.imap(roam_without_count_time, inputs), total=len(inputs), desc='roam_without_count_time'))
            else:
                with multiprocessing.Pool(processes=config.workers) as p: # API2Vec without freq
                    # corpus = list(tqdm(p.imap(roam_without_count, inputs), total=len(inputs), desc='roam_without_count'))
                    corpus = list(tqdm(p.imap(roam_without_count_analysis, inputs), total=len(inputs), desc='roam_without_count_analysis'))
    # for i in range(1):
    #     demo_inputs = random.choices(inputs, k=int(len(inputs) * 0.1))

    #     with multiprocessing.Pool(processes=config.workers) as p:
    #         # corpus = list(tqdm(p.imap(roam_without_count_analysis, demo_inputs), total=len(demo_inputs), desc='roam_without_count_analysis'))
    #         # corpus = list(tqdm(p.imap(random_walk_main_infos, demo_inputs), total=len(demo_inputs), desc='random_walk_main_infos'))
    #         # corpus = list(tqdm(p.imap(roam_without_time_analysis, demo_inputs), total=len(demo_inputs), desc='roam_without_time_analysis'))
    #         corpus = list(tqdm(p.imap(roam_without_count_time_analysis, demo_inputs), total=len(demo_inputs), desc='roam_without_count_time_analysis'))

    #     analysis_infos(corpus)
    # exit(0)
    return corpus
